...
触及产品/测试/依赖/验收脚本的 push/PR 使用 `Fast Verify`，在 Windows/Linux 原生环境分两阶段运行；纯文档和截图提交不会触发它：Stage 1
执行真实 `182/182 self-check`、`140/140 GUI acceptance`、聚焦回归并构建轻量
onefile；Stage 2 在不 checkout、不安装项目 Python 依赖的 fresh runner 上只执行
下载后的 onefile `182/182 self-check`。每个 job 上限十分钟。它不是三平台发布证据；
需要完整 onedir/onefile、GUI fresh acceptance 或 macOS 证据时，手动运行完整的
...
[`docs/验收矩阵.md`](docs/验收矩阵.md)，140 项逐项截图见
[`docs/验收截图索引.md`](docs/验收截图索引.md)。发布包不要求安装 Python、Graphviz 或
